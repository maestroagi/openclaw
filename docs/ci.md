...
| Runner                          | Jobs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ubuntu-24.04`                  | `security-fast`, manual CI dispatch and non-canonical repository fallbacks, CodeQL security and quality scans, workflow-sanity, labeler, auto-response, the standalone Docs workflow, the whole Install Smoke workflow, all configurable CI jobs in `github` mode, and the remaining light lanes plus rerun Blacksmith lanes in `hybrid` mode. The hosted planner profile expands QA Smoke to six parts, splits core oxlint across five stripes, and leaves extension/scripts lint plus optional UI and format checks in the existing `check-lint` row. |
...
Heavy lanes are `build-artifacts`, `check-sqlite-session-lifecycle`, `android`, `macos-node`, `macos-swift`, and `ios-build`. Hybrid also sends the eight-row hosted-planner `checks-ui-e2e` matrix, six-row QA Smoke matrix, `check-lint`, `check-test-types`, and `check-additional-extension-package-boundary` to their historical Blacksmith sizes on attempt 1. Every other configurable `ci.yml` lane stays hosted in hybrid, including preflight, the Node test matrix, the five core-lint stripes, the real-Gateway UI E2E job, Windows, the remaining lint/check rows, docs, and Python skills. Separate Opengrep workflows remain GitHub-hosted.
...
The changed-target PR plan reduces the common Node test burst from 29 Blacksmith registrations to one. Broad-risk PRs keep the 29-registration metadata-complete compact fallback; canonical pushes use 24 nondist compact registrations. With the `github` or `hybrid` hosted planner profile, the broader fallback uses 62 nondist hosted jobs and canonical pushes use 54; these jobs do not consume Blacksmith runner registrations. The widened hybrid attempt-1 set adds 17 Blacksmith registrations per full run: eight UI E2E rows, six QA Smoke rows, and three singleton checks.
