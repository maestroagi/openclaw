...
The changed-target PR plan reduces the common Node test burst from 29 Blacksmith registrations to one. Broad-risk PRs keep the 29-registration metadata-complete compact fallback; canonical pushes use 24 nondist compact registrations. While the outage backend is `github`, the broader fallback uses 41 nondist hosted jobs and canonical pushes use 36; these jobs do not consume Blacksmith runner registrations.
