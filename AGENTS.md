...
- A test that fails without a related change is a defect, never noise. Re-running a job, re-pushing, or refreshing a PR to get a green run is prohibited, as is landing over a red job you did not diagnose. Reproduce the failure (same shard order first), fix the cause at its owner (fixture, shared state, ordering, or product), add a regression that fails on the original defect, and record the root cause in the PR. If the cause belongs to another owner, open the fix there before your own PR lands; a sibling fix in flight is the only acceptable reason to proceed, and you cite it. Do not hide failures with retries, longer timeouts, weaker assertions, broader mocks, or altered baselines.
- **Red `main` is an emergency, not a queue item.** When `main` is broken, fix it in a fresh worktree, prove it with the exact failing check plus the focused tests (locally or on a Testbox), and push the fix straight to `main` when your permissions allow, so every open PR unblocks at once; open a PR only if the push is refused, and say so. Reruns do not clear a red `main`, and PRs whose merge ref went red need a fresh push after the fix lands.
...
- Complete the authorized workflow's review/merge gates; resolve substantive findings or explain rejections. Fix diff-caused failures. A failure outside your diff is not "unrelated" until you have identified its cause and the owning fix; document that fix by reference, never by rerunning. Verify remote outcomes before success or cleanup; uncertain writes require reconciliation, not blind retries.
