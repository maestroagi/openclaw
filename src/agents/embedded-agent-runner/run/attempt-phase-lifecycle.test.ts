...
describe("embedded attempt phase lifecycle state", () => {
  ...
  it("finalizes a deterministic discard as the rewound admitted user-only context-engine turn", async () => {
    const dir = tempDirs.make("openclaw-attempt-discard-terminal-anchor-");
    const target = {
      agentId: "main",
      sessionId: "session-discard",
      sessionKey: "agent:main:discard",
      storePath: path.join(dir, "sessions.json"),
    };
    await upsertSessionEntryCore(target, { sessionId: target.sessionId, updatedAt: 1 });
    const userMessage = { role: "user" as const, content: "keep this ask", timestamp: 1 };
    const persistedUser = await appendTranscriptMessage(target, {
      cwd: dir,
      eventId: "discard-user-1",
      message: userMessage,
      now: 1,
    });
    if (!persistedUser?.anchor) {
      throw new Error("expected discarded-turn user admission");
    }
    const recorder = createUserTurnTranscriptRecorder({
      message: userMessage,
      target: async () => undefined,
    ...
    await completeEmbeddedAttemptAfterTurn({
      ...
      state: {
        ...
        yieldAborted: false,
        sessionIdUsed: target.sessionId,
        messagesSnapshot: [userMessage] as never,
        prePromptMessageCount: 0,
        contextEngineAfterTurnCheckpoint: null,
      ...
      runtime: {
        ...
        anthropicPayloadLogger: null,
        hookAgentId: "main",
        diagnosticTrace: { traceId: "trace-discard", spanId: "span-discard" } as never,
        skillWorkshopAvailable: false,
        hookRunner: null,
        promptStartedAt: Date.now(),
      },
    ...
    expect(onContextEngineTurnCandidate).toHaveBeenCalledWith(
      expect.objectContaining({
        boundary: expect.objectContaining({
          admission: expect.objectContaining({ entryId: persistedUser.anchor.entryId }),
          terminal: expect.objectContaining({ entryId: persistedUser.anchor.entryId }),
        }),
      }),
    );
    expect(afterTurn).not.toHaveBeenCalled();
