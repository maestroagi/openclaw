...
import {
  prepareDeferredExactSourceReply,
  resolvesToDeferredExactSource,
  resolveTrustedDecisionChannel,
} from "./message-tool-exact-source.js";
...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  return {
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      const deferExactSourceDelivery =
        options?.deferSourceMessageToolDelivery === true &&
        resolvesToDeferredExactSource({
          action,
          args: params,
          channel: scope.channel ?? effectiveCurrentChannel.currentChannelProvider,
          accountId,
          sourceAccountId: trustedTurnContext?.requesterAccountId ?? agentAccountId,
          currentChannelProvider: effectiveCurrentChannel.currentChannelProvider,
          currentChannelId: effectiveCurrentChannel.currentChannelId,
          currentMessagingTarget: effectiveCurrentChannel.currentMessagingTarget,
          currentThreadTs,
      ...
      const toolContext =
        ...
        options?.sameChannelThreadRequired
          ...
          : undefined;
      if (deferExactSourceDelivery) {
        return prepareDeferredExactSourceReply({
          options,
          cfg,
          actionParams: params,
          accountId,
          turnContext: trustedTurnContext,
          toolContext,
          runId: deliveryRunId,
          ...
          sourceChannel: scope.channel ?? effectiveCurrentChannel.currentChannelProvider,
          requestedFinal: requestedSourceReplyFinal,
          abortSignal: signal,
        });
      }
      ...
      try {
        result = await runMessageActionForTool({
          ...
          deferredSourceReplyFinalIntent:
            options?.deferSourceMessageToolDelivery === true
